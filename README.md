...
- single-owner `TabNavigationStore` с lazy fallback, typed startup/save outcomes и frames;
...
Внутренний `SavedStateHandleTabNavigationStorage` оборачивает DTO в один defensive `ArrayList<String>` под отдельным key. Outer envelope хранит selected tab и ordered tab segments; каждый segment — полный length-prefixed payload существующего leaf-envelope, поэтому версии graph transport, graph snapshot, leaf transport, leaf snapshot и route payload не смешиваются. Storage принимает application-owned `RouteCodec`, возвращает typed `Missing`/`Restored`/`Rejected`, очищает только свой stale key и никогда не выбирает fallback.

`TabNavigationStore` — единственный process-local owner всего graph. Приложение хранит его в своём `ViewModel`; отдельный library-`ViewModel` и demo policy не требуются:

```kotlin
private val navigation = TabNavigationStore(
    savedStateHandle = handle,
    routeCodec = AppRouteCodec,
    fallbackStateFactory = TabNavigationStateFactory(::initialTabs),
)

val frames = navigation.frames
val navigationStart = navigation.startResult

fun dispatch(action: TabAction) = navigation.dispatch(action)
```
...
Каждая lambda `AnimatedContent` рендерит только переданный ей `branchState.tree`. Поэтому при переходе из expanded `Home -> Accounts -> Account(1)` в `AccountDetails(1)` outgoing-ветка сохраняет собственные `Home`, nested `Accounts` и sheet до конца exit, а incoming-ветка независимо рендерит details. Root и nested content используют точный `BackStackEntry.id` как Compose identity. Renderer стабилизирует content boundary неизменившегося entry отдельно от меняющегося `childContent`: локальное изменение nested-screen и замена nested destination не создают committed content pass родительского screen.
...
Граница остаётся обычной Compose: автоматически сохраняются только значения, допустимые для `rememberSaveable`/`Bundle`, либо значения с явно заданным `Saver`. Простой `remember`, domain state, большие и несериализуемые объекты этим контрактом не сохраняются. Instrumentation contract покрывает Activity recreation, но repository пока не заявляет проверку реального OS process kill для entry UI state.

Изоляция parent content определяется моделью: в пределах lifetime одной renderer composition, пока полный `BackStackEntry`, тип `Screen` и явный `parentInputsKey` binding-а не меняются, renderer удерживает первый instance и обновляет вложенный slot без повторного committed pass внешнего screen. Последующие instances с той же identity не являются каналом обновления. Данные родителя, которые должны обновляться независимо от navigation entry, следует передавать через observable Compose state, читаемый внутри `Screen.Content`, либо отражать изменением entry/type/input key. Это не обещание «screen никогда не recomposes»: собственный state, прочитанная observable-зависимость или новый explicit input key закономерно запускают его recomposition. Все три пути проверяют положительные instrumentation-controls.
